...
| 迭代4-H2 | db93303 | 50% | 37% | QueryProcessor语义扩展(3项改进) | ❌ -8pp 已回退 |
| 迭代5-RRF | 4d55eeb | 58% | 42% | RRF融合逻辑移入Reranker | ✅ 零回归 |
| **当前** | **646159f** | **58%** | **42%** | — | 距目标 -26pp/-36pp |

> **注**: 
> - 迭代4-H2 (原计划迭代4) 因权重路由导致 File@5=50% 暴跌 -8pp，已回退至 `3688993`。NDCG@5 提升了 +7.5pp 说明排序质量改善，但召回率严重下降。
> - 迭代5-RRF (RRF融入Reranker) 为纯结构重构，所有12项指标与基线完全一致，验证了架构可行性，为后续优化提供更干净的管线。
> - 迭代4-H4 (BM25索引增强) 已合入但未独立评估——其效果被迭代3-fix评估覆盖。
